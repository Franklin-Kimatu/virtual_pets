...
public class WaterMonsterTest {
    ...
    public void save_assignsIdToWaterMonster() {
        WaterMonster testWaterMonster = new WaterMonster("Bubbles", 1);
        testWaterMonster.save();
        WaterMonster savedWaterMonster = (WaterMonster) WaterMonster.all().get(0);
    ...
    public void save_savesPersonIdIntoDB_true() {
        ...
        testWaterMonster.save();
        WaterMonster savedWaterMonster =(WaterMonster) WaterMonster.find(testWaterMonster.getId());
    ...
    // adding the water level at the start as half full
    @Test
    public void waterMonster_instantiatesWithHalfFullWater(){
        WaterMonster testWaterMonster = new WaterMonster("Drippy",1);
        assertEquals(testWaterMonster.getWaterLevel(),(WaterMonster.MAX_WATER_LEVEL /2));
    }
    //water() increases the monster water level(testing the functionality
    @Test
    ...
    //assuming the water() exceeding the max.water level,it throws an exception that is caught here
    @Test(expected = UnsupportedOperationException.class)
    public void water_throwsExceptionIfWaterLevelsIsMaxVAlue(){
        for(int  i= WaterMonster.MIN_ALL_LEVELS;i<=(WaterMonster.MAX_WATER_LEVEL);i++){
            testWaterMonster.water();
        }
    }

    //checking the lastwatertime in the databse
    @Test
