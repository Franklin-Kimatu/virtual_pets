...
public class FireMonsterTest {
    ...
    public void save_assignsIdToFireMonster() {
        FireMonster testFireMonster = new FireMonster("Bubbles", 1);
        testFireMonster.save();
        FireMonster savedFireMonster =(FireMonster) FireMonster.all().get(0);
    ...
    public void save_savesPersonIdIntoDB_true() {
        ...
        testFireMonster.save();
        FireMonster savedFireMonster = (FireMonster) FireMonster.find(testFireMonster.getId());
    ...
    // test for ascertaining that a fire monster instianties with   half full firelevel
    @Test
    ...
    //testing for increasing the life of a firemonster by kiddling
    @Test
    ...
    //kingling would throw an exception if the kindling is exceeded
    @Test(expected = UnsupportedOperationException.class)
    public void kindling_throwsExceptionIfFireLevelsIsAtMaxValue(){
        FireMonster testFireMonster = new FireMonster("Smokey",1);
    ...
    //checking the deplete levels reducesAllLevels
    //we add the methods by overriding the methods from the abstract class
    @Test
    public void depletes_testFireMonster_reducesAllLevels(){
        testFireMonster.depleteLevels();
        assertEquals(testFireMonster.getFoodLevel(),(FireMonster.MAX_FOOD_LEVEL/2)-1);
    ...
    // checking that the kindling records time of last kindling in database
    @Test
